...
async function dragTaskToDate(
  ...
  const source = page
    .locator(
      '[data-testid="project-calendar-day"] button:has-text("' +
        taskTitle +
        '")',
    )
    .first();
  const target = page
    .locator(`[data-testid="project-calendar-day"][data-date="${targetDate}"]`)
    .first();
...
async function setupProjectShellRoutes(page: Page) {
...
test.describe("Elastic Schedule Timeline", () => {
  ...
  test("cascades dependent tasks and triggers reschedule suggestion", async ({
    page,
  }, testInfo) => {
    ...
    await setupProjectShellRoutes(page);

    await page.route(
      "**/api/v1/projects/project-1/cascade-reschedule",
      async (route) => {
        const request = route.request();
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            suggestion: {
              summary:
                "I slid the downstream tasks, but Weeding hit a storm, so I pushed it an extra 2 days.",
    ...
    await expect(
      page.getByRole("heading", { name: "2026年2月" }),
    ).toBeVisible();
    ...
    // Drag the seeded open task from 2026-02-16 to 2026-02-17
    await dragTaskToDate(page, "Open baseline task", "2026-02-17");
    ...
    expect(cascadePayload).toMatchObject({
      ...
      toDate: expect.stringContaining("2026-02-17"),
    });
