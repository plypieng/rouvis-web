...
async function setupChatkitRoutes(
  page: Page,
...
async function setupProjectDetailRoutes(page: Page) {
...
async function setupProjectsListRoutes(
  page: Page,
...
async function expectClassContains(locator: Locator, classToken: string) {
...
test.describe('Mobile workflow UX regression', () => {
  ...
  test('stacks dashboard recovery actions cleanly on mobile', async ({ page }) => {
    ...
    expect((thirdQuickActionBox?.y ?? 0) - (firstQuickActionBox?.y ?? 0)).toBeGreaterThan(12);
  });
