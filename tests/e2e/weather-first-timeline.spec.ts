...
function buildDailyForecastRange(
  startDate: string,
  endDate: string,
  rainyDate: string,
) {
  ...
  const safeEnd = Number.isFinite(end.getTime())
    ? end
    : new Date(safeStart.getTime());
...
async function setupCommonClientRoutes(page: Page) {
  ...
  await page.route("**/api/v1/knowledge/crops**", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
...
async function dragTaskToDate(
  page: Page,
  taskTitle: string,
  targetDate: string,
) {
  const source = page
    .locator('[data-testid="calendar-date"] [data-testid="scheduled-item"]')
    .filter({ hasText: taskTitle })
    .first();
  const target = page
    .locator(`[data-testid="calendar-date"][data-date="${targetDate}"]`)
    .first();
...
test.describe("Weather-first timeline", () => {
  ...
  test("applies rainy-day weather metadata in standalone calendar", async ({
    page,
  }) => {
    ...
    const rainyDay = page.locator(
      '[data-testid="calendar-date"][data-date="2026-02-16"]',
    );
    ...
    await expect(
      rainyDay.locator('[data-testid="calendar-weather-icon"]'),
    ).toBeVisible();
  });

  test("requires explicit force-confirmation when dropping weather-sensitive task on rainy day", async ({
    page,
  }, testInfo) => {
    ...
    await expect(
      page.getByTestId("weather-drop-warning-message"),
    ).toContainText("85%");
  ...
  test("renders weather risk metadata in project month grid", async ({
    page,
  }, testInfo) => {
    await setupCommonClientRoutes(page);
    ...
    await page.route(
      "**/api/v1/projects/project-1/reschedule-suggestion",
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
    ...
    const rainyProjectCell = page
      .locator(
        '[data-testid="project-calendar-day"][data-weather-risk="rainy"]',
      )
      .first();
    await expect(rainyProjectCell).toBeVisible();
    await expect(
      rainyProjectCell.locator('[data-testid="project-calendar-weather-icon"]'),
    ).toBeVisible();
  });
});
