...
export async function GET(request: NextRequest) {
  try {
    ...
    appendQueryParam(params, 'startDate', request.nextUrl.searchParams.get('startDate'));
    appendQueryParam(params, 'endDate', request.nextUrl.searchParams.get('endDate'));

    const response = await fetch(
      `${backendUrl}/api/v1/weather/forecast/daily${params.toString() ? `?${params.toString()}` : ''}`,
      {
    ...
    console.error('Weather daily forecast API proxy error:', error);
