...
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/api/v1/projects/${projectId}/cascade-reschedule`, {
      method: 'POST',
      headers: {
    ...
    console.error('Project cascade-reschedule proxy POST error:', error);
    return toApiErrorResponse({
      status: 500,
      code: 'INTERNAL_ERROR',
      message: 'Failed to run cascade reschedule',
      requestId,
    });
  }
}
