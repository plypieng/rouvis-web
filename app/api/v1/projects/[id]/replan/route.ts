...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${BACKEND_URL}/api/v1/projects/${projectId}/replan`, {
      method: 'POST',
      headers: {
    ...
    console.error('Project replan proxy POST error:', error);
    return toApiErrorResponse({
      status: 500,
      code: 'INTERNAL_ERROR',
      message: 'Failed to run project replan',
      requestId,
    });
  }
}
