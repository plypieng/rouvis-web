...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/agents/analyze-plant-photo`, {
      method: 'POST',
      headers: {
    ...
    console.error('Agents analyze-plant-photo proxy error:', error);
    return NextResponse.json({ error: 'Failed to analyze plant photo' }, { status: 500 });
  }
}
