...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/agents/estimate-planting-date`, {
      method: 'POST',
      headers: {
    ...
    console.error('Agents estimate-planting-date proxy error:', error);
    return NextResponse.json({ error: 'Failed to estimate planting date' }, { status: 500 });
  }
}
