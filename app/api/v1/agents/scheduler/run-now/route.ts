...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/agents/scheduler/run-now`, {
      method: 'POST',
      headers: {
    ...
    console.error('Scheduler run-now proxy error:', error);
    return NextResponse.json({ error: 'Failed to queue scheduler run' }, { status: 500 });
  }
}
