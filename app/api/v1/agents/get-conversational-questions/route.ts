...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/v1/agents/get-conversational-questions`, {
      method: 'POST',
      headers: {
    ...
    console.error('Agents get-conversational-questions proxy error:', error);
    return NextResponse.json({ error: 'Failed to fetch conversational questions' }, { status: 500 });
  }
}
