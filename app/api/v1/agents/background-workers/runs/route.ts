...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(
      `${BACKEND_URL}/api/v1/agents/background-workers/runs${params.toString() ? `?${params.toString()}` : ''}`,
      {
        headers: {
    ...
    console.error('Background workers proxy GET error:', error);
    return NextResponse.json({ error: 'Failed to fetch background workers' }, { status: 500 });
  }
}
