...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/api/v1/agents/schedule-generation/runs/${runId}`, {
      headers: {
    ...
    console.error('Schedule generation run status proxy error:', error);
    return toApiErrorResponse({
      status: 500,
      code: 'INTERNAL_ERROR',
      message: 'Failed to fetch schedule generation run',
      requestId,
    });
  }
}
